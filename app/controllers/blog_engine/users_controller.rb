...
module BlogEngine
  class UsersController < ApplicationController
    skip_before_action :login_required, only: %i[forgot_password login signup]
      ...
      begin
        User.transaction do
          ...
          if @user.save
            ...
            flash.now[:notice] = t(:user_updated_password, s: @user.email.to_s)
          end
        end
    ...
    def edit
      ...
      return unless params['user']['form']
    ...
    def destroy(user)
      user.destroy
      flash[:notice] = t(:user_delete_finished, s: user.email.to_s)
