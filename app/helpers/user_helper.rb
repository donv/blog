module UserHelper
  ...
  def form_input(helper_method, field_name, options = {}, form_name = nil)
    ...
    when /^.*button$/
      prompt = t(:"#{controller.controller_name}_#{field_name}_button")
      <<-EOL
...
<<-EOL
  ...
  def button_helper(name, options = {})
    label = t(:"#{controller.controller_name}_#{name}_button")
    "#{self.send(:submit_tag, label, options)}".html_safe
  end

  def link_helper(name, options = {})
    raise ArgumentError if name.nil?
    label = t(:"#{controller.controller_name}_#{name}_link")
    "#{self.send(:link_to, label, options)}".html_safe
  end
  ...
  def head_helper(options = {})
    label = t(:"#{controller.controller_name}_#{controller.action_name}_head")
...
EOL
  ...
  def message_helper(name)
    t(:"#{controller.controller_name}_#{name}_message")
  end
