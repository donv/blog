...
class User < ActiveRecord::Base
  ...
  def generate_security_token(hours = nil)
    if not hours.nil? or self.security_token.nil? or self.token_expiry.nil? or 
        (Clock.now.to_i + token_lifetime / 2) >= self.token_expiry.to_i
  ...
  def set_delete_after
    ...
    write_attribute('delete_after', Clock.at(Clock.now.to_i + hours * 60 * 60))
  ...
  def new_security_token(hours = nil)
    ...
    write_attribute('token_expiry', Clock.at(Clock.now.to_i + token_lifetime(hours)))
    update_without_callbacks
    return self.security_token
  end
